...
>
    <form id="facialVerificationForm" method="POST" action="{{ route('driver-verification.store') }}" class="flex flex-1 flex-col min-h-0">
        ...
        <footer
            class="shrink-0 flex flex-col items-center gap-4 px-4 pt-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-gradient-to-t from-black via-black/95 to-transparent"
        ...
        >
            ...
            <div id="dvvStepThumbsRow" class="hidden w-full max-w-md grid grid-cols-3 gap-2">
    ...
    (() => {
        ...
        function setMode(next) {
            ...
            if (live) {
                ...
                stepThumbsRow?.classList.add('hidden');
                stepThumbsRow?.classList.add('pointer-events-none');
                startAlignmentLoop();
            } else {
                ...
                stepThumbsRow?.classList.remove('pointer-events-none');
                stopAlignmentLoop();
            }
        }
        ...
        updateStepUi();
        resetAllStepPreviews();
        if (stepThumbsRow) stepThumbsRow.classList.add('hidden', 'pointer-events-none');
        showPermissionGate(
